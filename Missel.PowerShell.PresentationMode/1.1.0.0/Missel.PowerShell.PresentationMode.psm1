...
function Enable-PresentationMode {
    ...
    Begin {
        ...
        if ($LogFile) { Test-LogFilePath -LogFile $LogFile | Out-Null }
    }
        ...
        if ($Force -or $PSCmdlet.ShouldProcess($env:COMPUTERNAME, "Enabling Presentation Mode.")) {
            try {
                ...
                if ($result -eq 0 -or $null -eq $result) {
                    ...
                    Write-Verbose "$LastUpdated - Presentation Mode enabled. Current State: $state"

                    if ($LogFile) {
                        Add-Content -Path $LogFile -Value "$LastUpdated - Presentation Mode enabled. Current State: $state"
                    }
        ...
        if ($Force -or $PSCmdlet.ShouldProcess($env:COMPUTERNAME, "Disabling Presentation Mode.")) {
            ...
            try {
                ...
                if ($result -eq 0 -or $null -eq $result) {
                    ...
                    $Script:currentState = $false
                    $LastUpdated = (Get-Date -Format "yyyy-MM-dd HH:mm:ss")
                    Write-Verbose "$LastUpdated - Presentation Mode disabled. Current State: $state"

                    if ($LogFile) {
                        Add-Content -Path $LogFile -Value "$LastUpdated - Presentation Mode disabled. Current State: $state"
                    }
